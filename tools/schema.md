...
responsiveness, adoption, longevity, governance, risk_license), each graded `A`–`E`, `?` or `N/A`,
plus an aggregate `overall` and the raw measured values. `?` and `N/A` are different claims and are
never interchangeable: `?` means the measurement was attempted and failed (the gap may be ours, and
the axis stays in the denominator), `N/A` means the axis asks a question this artifact type cannot
answer at all (it leaves the denominator). Their reasons are emitted under separate `unknowns:` and
`not_applicable:` keys; see [`docs/health-rubric.md`](../docs/health-rubric.md) §2.3c. It is the machine SSOT for the JoJo-stand-stats card
...
conclusion; render the scored-axis coverage (`scored_axes`/`applicable_axes`) so incomplete
aggregates are visible. The denominator is `applicable_axes`, not a hardcoded 6 — counting an axis
that does not apply to the type would understate a page's real coverage.
...
- **Health column projection (never hand-edit).** The `Health` / `健康度` cell of every summary row
  ...
  page's frontmatter `health:` block, rendered `overall (scored_axes/applicable_axes)`
  (ZH: fullwidth parens).
