...
- **The handle you have is not the handle they used there.** Sherlock keys the entire sweep on one exact string, so it can only ever reach the subset of sites where that string was reused; the platforms where your target picked a different name are invisible to it, and adding site coverage does not fix that. Switch the *key*, not the tool: probe the email instead with [socialscan](socialscan.md) (server-side available/taken on ~11 platforms) or a re-verified [holehe](holehe.md) (120+ sites), because an address survives a platform switch far more often than a handle does. [推断]
...
|---|---|---|---|
| [Maigret](maigret.md) | ✅ | Choose Maigret when the deliverable is a dossier (extracted IDs, recursion, reports, 3000+ sites, Tor/I2P); choose Sherlock when the deliverable is a fast, simple, auditable existence list across 481 sites. | Maigret pays for depth with a heavy poetry stack and slower scans; Sherlock pays for simplicity with coarser profile-page signals and no extraction. |
| [socialscan](socialscan.md) | ✅ | Choose socialscan when you can key on an email, or need registration-grade available/taken accuracy on ~11 platforms; choose Sherlock when all you have is a handle and you want 481-site coverage of where that exact string was reused. | socialscan answers with the platform's own verdict instead of a page-shape guess, and it accepts an email — the more stable cross-platform key; it pays for that with 40× fewer platforms. |
...
- **Site database:** declarative `data.json` (481 entries as of 2026-09) mapping each site to a profile URL pattern, error-message matchers, and metadata; contributors extend it via PR.
...
- Outbound HTTPS to 481 sites; optional Tor daemon for `--tor`/`--unique-tor`; optional HTTP/SOCKS proxy via `--proxy`.
...
- [未验证] Per-site module health was not live-tested — only the 481-entry total was counted against master `sherlock_project/resources/data.json`. The count was 482 top-level keys, one of which is the `$schema` metadata key, not a site.
...
- [推断] "An address survives a platform switch more often than a handle does" is a mechanism inference from how register/forgot-password endpoints are keyed, not a measured rate — no per-platform handle-reuse statistics were found to check it against.
