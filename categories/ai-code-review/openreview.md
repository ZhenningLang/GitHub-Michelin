...
language: TypeScript
license: NOASSERTION
maturity: active, ~1,450 stars (as of 2026-07)
...
health:
  schema: 1
  computed_at: 2026-09-23T07:39:34Z
  overall: D
  overall_score: 1.8
  scored_axes: 5
  applicable_axes: 6
...
You're choosing open-source infrastructure for a task that falls into `ai-code-review` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for OpenReview when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because OpenReview was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on OpenReview.
...
|---|---|---|---|
| [Claude Code Security Review](claude-code-security-review.md) | ✅ | When you need the established in-index option for this category, compare it against OpenReview before switching. | OpenReview is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose OpenReview only after verifying the repo-specific caveats below. |
| [Metis](metis.md) | ✅ | When you need the established in-index option for this category, compare it against OpenReview before switching. | OpenReview is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose OpenReview only after verifying the repo-specific caveats below. |
| [Open Code Review](open-code-review.md) | ✅ | When you need the established in-index option for this category, compare it against OpenReview before switching. | OpenReview is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose OpenReview only after verifying the repo-specific caveats below. |
| [PR-Agent](pr-agent.md) | ✅ | When you need the established in-index option for this category, compare it against OpenReview before switching. | OpenReview is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose OpenReview only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-03-06T16:44:33Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-03-06T16:44:33Z` as of 2026-07-07.
- **Adoption snapshot:** ~1,450 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
