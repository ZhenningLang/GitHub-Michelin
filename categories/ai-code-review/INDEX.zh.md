...
| **Claude Code Security Review** | 当你想用 Claude 在可信 PR 上做上下文感知的安全审查、且接受按 token 计费与非确定性结果时使用。 | B（4/6） | [→](claude-code-security-review.zh.md) |
| **React Doctor** | 当 coding agent 在写 React、你想要对 React 特有反模式做确定性、可重复的检查时用它。 | B（5/6） | [→](react-doctor.zh.md) |
| **PR-Agent** | 🚀 PR Agent: The Original Open-Source PR Reviewer.  This project It is not the Qodo free tier. | A（5/6） | [→](pr-agent.zh.md) |
| **Metis** | Metis is an open-source, AI-driven tool for deep security code review | B（5/6） | [→](metis.zh.md) |
| **OpenReview** | An open-source, self-hosted AI code review bot powered by Vercel. | D（5/6） | [→](openreview.zh.md) |
...
| [Claude Code Security Review](claude-code-security-review.zh.md) | ✅ | B（4/6） | 当你想用 Claude 在可信 PR 上做上下文感知的安全审查、且接受按 token 计费与非确定性结果时使用。 |
