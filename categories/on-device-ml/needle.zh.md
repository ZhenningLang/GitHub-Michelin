...
- **你要在 GPU 上服务大量并发用户。** 这是按设备部署的模型加 SDK，没有批处理服务端。托管服务改用 vLLM 或 TGI（或直接调用云端 API）配更大的函数调用模型。
- **你要一个能承载任意模型的运行时。** Needle 只提供单一模型族，不是通用推理引擎。如果优先考虑模型选择，用 llama.cpp、Ollama 或 [LiteRT-LM](litert-lm.zh.md)。
- **你要在困难或模糊请求上追求最高工具调用准确率。** 当准确率是硬约束且能接受一次网络往返时，改用云端前沿函数调用（OpenAI / Gemini），或 Functionary 这类更大的开源函数调用模型，并接受更大的体积。
...
| 替代品 | 是否收录 | 我们的评价 | 取舍 |
|---|---|---|---|
| llama.cpp + 小指令模型 GGUF | 未收录 | 需要一个运行时承载任意小模型、并要 grammar 约束输出与完整平台控制时，选 llama.cpp；希望工具调用、grammar 和 confidence 行为开箱即得、而不是自己拼装时，选 Needle。 | 通用运行时，模型选择极多且本地可控；但 prompt、tool schema、解析器和 grammar 都要自己拼，也没有校准过的 confidence 和 grounding 校验。 |
...
| Functionary | 未收录 | 想要一个完整的开源函数调用模型、在 CPU/GPU 服务器上配丰富的工具/grammar 生态时，选 Functionary；需要模型压缩到个位数 MB 放到端侧时，选 Needle。 | 更通用的工具调用能力和更大的生态；但体积大几个数量级，不是可穿戴/MCU 级的占用。 |
