...
health:
  schema: 1
  computed_at: 2026-09-22T17:44:56Z
  overall: B
  overall_score: 3.4
  scored_axes: 5
  applicable_axes: 6
...
![deeptutor — 健康度雷达](../../assets/health/deeptutor.zh.svg)

## 何时使用

你正在评估 `education-tutoring` 方向的任务，需要把一个真实仓库纳入 GitHub-Michelin 候选，而不是只在 backlog 里看到一个名字。当上游描述贴合任务、许可证和维护画像经核验后可接受，并且采用公共项目比自写一次性方案更合适时，可以把 DeepTutor 纳入候选。
...
- **仓库形态：** `HKUDS/DeepTutor`；本首版页面尚未穷尽读取所有依赖清单。
- **默认分支快照：** 最后 push `2026-07-09T15:24:10Z`，archived 为 `false`。
...
- **维护快照（2026-07-16）：** GitHub 返回 `archived=false`，`pushed_at=2026-07-09T15:24:10Z`。
- **采用快照：** 2026-07 约 26,528 个 GitHub stars；这是有噪声的信号，低 star 项目只要是真实且相关，也会被纳入。
