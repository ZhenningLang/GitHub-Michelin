...
| Hosted sandbox APIs (E2B, Modal) | 未收录 | Choose E2B/Modal when you want sandboxes or serverless containers as a managed service and will not run Kubernetes; choose Agent Substrate when agent memory and state must stay inside your own cluster and bucket. | Hosted removes the entire ops burden — and the density problem, since the provider meters it for you — but you give up self-hosting, node-level control, and keeping suspend/resume state in your own object store. |
| gVisor / Kata Containers / Firecracker (alone) | 未收录 | Choose a sandbox runtime directly when you only need the isolation primitive and are willing to build the lifecycle; choose Agent Substrate when you want lifecycle, scheduling, snapshotting and routing already built on top of those primitives. | Substrate depends on exactly these for isolation (gVisor today, micro-VM via cloud-hypervisor or Kata as the other class), so this compares a component to a system: the primitives give you nothing about multi-tenant density or wake latency. |
| Plain Kubernetes Pods + HPA / Knative | 未收录 | Choose plain Pods when sessions are few, long-running, or your compute bill is acceptable; choose Agent Substrate when the bill is dominated by idle sandboxes and cold starts. | Pods are the boring, supported, universally understood path with no new control plane; what you pay is one pod per session and the wake-up latency — the exact two things Substrate was written to fix. |
| kagent (CNCF Sandbox) | 未收录 | Choose kagent when you want a Kubernetes-native *framework* for building and operating agents as workloads; choose Agent Substrate when you already have agents and need the runtime beneath them to multiplex stateful sandboxes. | kagent consumes this layer rather than competing with it (its own material describes running sandboxed, stateful agent workloads on Substrate), so they stack; kagent gives you agent CRDs and orchestration, not suspend/resume density. |
