...
> 分类节点。往现有 coding-agent harness 上外挂能力——跨 agent 安装技能包，以及给只有图形界面的软件造一个 agent 能用的命令面。
...
**扩展运行中 agent 能触达什么、能做什么**的手段：技能包的安装与发现，以及为只有图形界面的软件造命令面。不含 prompt/技能集合本体（见 `agent-skills`），也不含 agent 框架与运行时（见 `agent-frameworks`）。
