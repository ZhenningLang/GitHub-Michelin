...
- **维护：** Grade `?`——本次评分未能读取默认分支的活跃度（`recency_unreadable`）。GitHub 仓库级 `pushed_at` 是 2026-03-07，但它来自默认分支之外的工作；最新稳定版仍是 2025 年 8 月的 v5.1.2。
...
- **长青度：** Grade `?`——本次评分未能找到长青度数据（`not_found`）；缺少可读的当前稳定版本线信号，仅凭年龄无法评出 Lindy 信号。[推断]
