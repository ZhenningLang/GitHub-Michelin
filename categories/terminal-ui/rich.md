...
tags: [terminal-ui, rich, library]
language: Python
license: MIT
maturity: active, ~56,788 stars (as of 2026-07)
...
health:
  schema: 1
  computed_at: 2026-09-22T17:07:12Z
  overall: B
  overall_score: 3.17
  scored_axes: 6
  applicable_axes: 6
  ...
  axes:
    ...
    adoption:
      ...
      raw:
        ...
        downloads_last_month: 480190119
        graph_tier: A
        volume_tier: A
...
You're choosing open-source infrastructure for a task that falls into `terminal-ui` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for Rich when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because Rich was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on Rich.
...
|---|---|---|---|
| [Alacritty](alacritty.md) | ✅ | When you need the established in-index option for this category, compare it against Rich before switching. | Rich is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Rich only after verifying the repo-specific caveats below. |
| [ART](art.md) | ✅ | When you need the established in-index option for this category, compare it against Rich before switching. | Rich is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Rich only after verifying the repo-specific caveats below. |
| [asciify](asciify.md) | ✅ | When you need the established in-index option for this category, compare it against Rich before switching. | Rich is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Rich only after verifying the repo-specific caveats below. |
| [asciimatics](asciimatics.md) | ✅ | When you need the established in-index option for this category, compare it against Rich before switching. | Rich is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Rich only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-06-23T03:10:19Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-06-23T03:10:19Z` as of 2026-07-06.
- **Adoption snapshot:** ~56,788 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
