...
- **采用度——现实里很广，但被雷达低估。** 该轴测出来是 `D`，因为它只看包注册表与 GitHub 依赖图（conda-forge 的 `freecad`，上月 508,217 次下载，4 个依赖仓库）——这不适配一个桌面 C++ CAD 应用：它的采用体现在发行版软件包、安装器和插件生态里，而不是作为库被依赖。更广的证据是分发与社区：发行版仓库、官方 Windows/macOS 安装包、AppImage、Flathub 与 Snap，以及庞大的 wiki/论坛/Crowdin 翻译与宏/工作台生态。
