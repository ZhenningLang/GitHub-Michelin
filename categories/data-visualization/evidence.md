...
tags: [data-visualization, evidence, framework]
language: JavaScript
license: MIT
maturity: active, ~6,533 stars (as of 2026-07)
last_verified: 2026-07-06
type: framework
upstream:
  pushed_at: 2026-02-18T20:43:42Z
  default_branch: main
  default_branch_sha: bdf2ce10040f81dbfcd43acde438052cc25b8e5c
  archived: false
health:
  schema: 1
...
You're choosing open-source infrastructure for a task that falls into `data-visualization` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for Evidence when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because Evidence was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on Evidence.
...
|---|---|---|---|
| [Apache Superset](superset.md) | ✅ | When you need the established in-index option for this category, compare it against Evidence before switching. | Evidence is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Evidence only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-02-18T20:43:42Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-02-18T20:43:42Z` as of 2026-07-06.
- **Adoption snapshot:** ~6,533 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
