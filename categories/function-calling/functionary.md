...
health:
  schema: 1
  computed_at: 2026-09-22T16:18:55Z
  overall: B
  overall_score: 2.75
  scored_axes: 4
  applicable_axes: 6
...
- **You want an actively maintained repo with issue response.** Recent pushes notwithstanding, the project is declared dead. If maintenance matters, choose a maintained engine ([vLLM](../llm-inference/serving-engines/vllm.md), [SGLang](../llm-inference/serving-engines/sglang.md)) and a maintained model.
...
| [vLLM](../llm-inference/serving-engines/vllm.md) | ✅ | Pick vLLM when you are serving a current function-calling model yourself; Functionary's bundled `server_vllm.py` is a frozen 2025-era snapshot of exactly this path. | A maintained serving engine with a live ecosystem versus a pinned, unmaintained server script; same architecture, opposite maintenance posture. |
