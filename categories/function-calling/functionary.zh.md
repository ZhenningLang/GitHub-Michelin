...
对任何真正要上线的系统，把这份设计拿出去，换成**当前**的模型：在 [vLLM](../llm-inference/vllm.zh.md) 或 [SGLang](../llm-inference/sglang.zh.md) 上跑现代函数调用模型；体积必须压到个位数 MB 时用端侧的 [Needle](../on-device-ml/needle.zh.md)；准确率是硬约束时调用云端前沿 API。选这层抽象，只是因为你想要这套模式；模型去别处选。

## 何时不用

- **任何生产部署。** 维护者在 README 上加了明确的废弃声明：不再更新、不再修 bug、不提供支持，代码/模型/文档都被描述为非常旧的快照。改用维护中的服务路径（[vLLM](../llm-inference/vllm.zh.md) 或 [SGLang](../llm-inference/sglang.zh.md)）配当前模型。
...
- **你要一个活跃维护、有 issue 响应的仓库。** 尽管近期仍有 push，项目已被声明终止。维护性重要时，选维护中的引擎（[vLLM](../llm-inference/vllm.zh.md)、[SGLang](../llm-inference/sglang.zh.md)）配维护中的模型。
...
| [vLLM](../llm-inference/vllm.zh.md) | ✅ | 你要自己服务当前函数调用模型时，选 vLLM；Functionary 自带的 `server_vllm.py` 不过是这条路径 2025 年前后的冻结快照。 | 维护中的服务引擎与活跃生态，对一个锁定版本、无人维护的服务脚本；架构相同，维护姿态相反。 |
| [SGLang](../llm-inference/sglang.zh.md) | ✅ | 同理选 SGLang——约束/结构化生成加高速服务，用来跑工具调用模型——而不是 Functionary 已废弃的 SGLang 集成。 | RadixAttention 与结构化生成是当下的默认；Functionary 的价值在于记录模式，而不在运行它。 |
