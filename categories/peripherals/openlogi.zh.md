...
- **采用与生态——分发渠道真实，支持负载未消化。** 官方 Homebrew cask、发行版包、NixOS 模块、签名 MSI，21,660 star、701 fork。按本索引的雷达口径，**采用广度评级为 D，依赖方数量为 0**——该轴看的是依赖图与安装触达，一个没有库依赖方的终端桌面应用天然在这里得分低，不代表没人用。真正的反面是 4 个月大的仓库上有 587 个未关闭的 issue 加 PR；元数据无法区分增长压力与积压。`[推断]`
