...
| **Mouser** | 当你想要在 Windows／macOS／Linux 上用便携压缩包按应用重映射罗技 HID++ 鼠标，不需要安装器、账号或服务，并且不要求配对、键盘、摄像头或按设备映射时用它。 | B（5/6） | [→](mouser.zh.md) |
| **logiops** | 在 Linux 上，当你想要一个读单份声明式 `/etc/logid.cfg` 的 root systemd 守护进程而不是 GUI 时用它——接受只支持 HID++ 2.0+ 鼠标、没有应用感知，且开发自 2024 年起实际已停。 | C（4/6） | [→](logiops.zh.md) |
...
| [Mouser](mouser.zh.md) | ✅ | B（5/6） | 只管鼠标且便携：三个系统解压即用，支持按应用 profile；没有配对、映射为全局、7 个月历史。 |
| [logiops](logiops.zh.md) | ✅ | C（4/6） | 守护进程加配置文件，没有 GUI：在 Linux 上稳定、可打包，但开发近乎休眠，且只支持 HID++ 2.0+。 |
