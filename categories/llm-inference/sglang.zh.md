...
| [Ollama](ollama.zh.md) / [llama.cpp](llama-cpp.zh.md) | ✅ | 数据中心吞吐服务带结构化生成选 SGLang；轻量级本地/边缘 CPU 或消费级 GPU 推理选 Ollama/llama.cpp。 | 可移植的 C/C++ 推理引擎（GGUF），可在 Mac 和手机等任何地方运行；不是数据中心多 GPU 吞吐引擎。 |
