...
| 项目 | 何时用 | 健康度 | 页面 |
| --- | --- | --- | --- |
| **llama.cpp** | 当你想要那个无依赖的 C/C++ 上游引擎、跑在目前最宽的硬件矩阵上、既能嵌入也能本地起服务时用它——接受没有 semver、没有模型管理、只有 flag 级 API。 | B（6/6） | [→](llama-cpp.zh.md) |
| **Ollama** | 当你想要托管的本地模型仓库，配 OpenAI/Anthropic 兼容 API、Docker 部署与客户端 SDK 时用它——接受包装层的参数子集、4096 token 的默认上下文，以及没有服务级批处理。 | A（4/6） | [→](ollama.zh.md) |
...
| 选项 | 是否收录 | 健康度 | 一句话取舍 |
| --- | --- | --- | --- |
| [llama.cpp](llama-cpp.zh.md) | ✅ | B（6/6） | 上游 GGUF 引擎，后端矩阵最宽且无运行时依赖；没有 semver、没有模型管理、只有 flag 级控制。 |
| [Ollama](ollama.zh.md) | ✅ | A（4/6） | 托管式模型仓库，配 OpenAI/Anthropic 兼容 API、Docker、MLX 与 llama.cpp 双 runner，客户端生态最深；代价是包装层参数子集与 4096 token 默认上下文。 |
