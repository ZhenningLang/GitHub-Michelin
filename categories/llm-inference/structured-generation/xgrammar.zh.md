...
多数开源 LLM 服务栈已经在底层运行的语法约束解码引擎：把 JSON Schema、正则、EBNF 或 Lark 语法按模型的 tokenizer 编译一次，然后在每一步采样时掩掉不符合语法的 token，让输出无法越出结构。

![xgrammar — 健康度雷达](../../../assets/health/xgrammar.zh.svg)

## 何时使用

你在做一个必须输出机器可解析结果的 agent 或服务——给下游 API 用的 JSON、按模型自己的对话模板拼出的工具调用、某种 DSL 或代码片段——而你掌握模型的 logits，因为模型是你自己跑的（HuggingFace `transformers`，或 [vLLM](../serving-engines/vllm.zh.md)、[SGLang](../serving-engines/sglang.zh.md) 这类服务引擎）。「解析失败就重试」不可接受：它烧 token、加延迟，而且在最不能出错的时候仍然偶尔给你一个畸形载荷。
...
XGrammar 在每一步解码时判断：按你要的结构，哪些 token 仍然合法，然后把其余 token 从候选里去掉再采样——于是模型只能吐出符合语法的文本。你把结构描述一次、把模型的 tokenizer 交给它，它就会针对这个确切词表预编译出合法 token 集合；这件事对每一对「语法 + 模型」只做一次并缓存，所以重复或共享的 schema 几乎不花代价。运行时的状态机很小且有状态：接受上一步采样出的 token，取下一步的掩码，把掩码作用到 logits 上，再采样一次。你与它的分工很干净——**prompt、模型和生成循环归你**（而且你仍应在 prompt 里写清所需结构，因为掩码只作用于采样阶段，没法让模型真心想给对答案），**语法编译、token 掩码和保持输出不越界的状态机归它**。如果你通过 vLLM、SGLang、TensorRT-LLM 或 MLC-LLM 提供服务，就不必自己接：这些引擎在它们各自的结构化输出选项背后调用 XGrammar。
