...
- **Adoption: the strongest signal on the page** — 94,383,978 PyPI downloads in the last month and 3,530 dependent repositories (health radar, 2026-09-18); 5,722 stars / 1,309 forks; 522 open issues. Its adoption is also indirect and load-bearing: [Office-Word-MCP-Server](office-word-mcp-server.md) declares `python-docx>=1.1.2`, and Word skill packs across harnesses wrap it rather than replace it. [推断] At ~94M downloads/month the project is effectively unmaintained-but-critical: it cannot be abandoned without a large ecosystem noticing, which is itself a form of durability.
