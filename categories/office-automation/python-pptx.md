...
| Office-PowerPoint-MCP-Server | 未收录 | Archived by its author on 2025-12-31 (1,852 stars, API-verified); do not pick it for new work — use python-pptx directly, since the server was a thin MCP wrapper over this same library layer. | It offered a ready-made MCP tool schema for LLM clients; that convenience is now unmaintained, and the wrapper added no capability python-pptx lacks. |
