...
tags: [database, data, valkey, service]
language: C
license: BSD-3-Clause
maturity: active, ~26,448 stars (as of 2026-07)
last_verified: 2026-07-06
type: service
upstream:
  pushed_at: 2026-07-06T11:32:51Z
  default_branch: unstable
  default_branch_sha: 5bcc9f7440d78976e6bdf5804d7dc74338313d95
  archived: false
health:
  schema: 1
...
You're choosing open-source infrastructure for a task that falls into `databases` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for Valkey when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because Valkey was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on Valkey.
...
|---|---|---|---|
| [ClickHouse](clickhouse.md) | ✅ | When you need the established in-index option for this category, compare it against Valkey before switching. | Valkey is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Valkey only after verifying the repo-specific caveats below. |
| [DBeaver](../database-clients/dbeaver.md) | ✅ | When you need the established in-index option for this category, compare it against Valkey before switching. | Valkey is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Valkey only after verifying the repo-specific caveats below. |
| [Debezium](../data-sync/debezium.md) | ✅ | When you need the established in-index option for this category, compare it against Valkey before switching. | Valkey is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Valkey only after verifying the repo-specific caveats below. |
| [DuckDB](duckdb.md) | ✅ | When you need the established in-index option for this category, compare it against Valkey before switching. | Valkey is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Valkey only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `unstable`, last pushed `2026-07-06T11:32:51Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-07-06T11:32:51Z` as of 2026-07-06.
- **Adoption snapshot:** ~26,448 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
