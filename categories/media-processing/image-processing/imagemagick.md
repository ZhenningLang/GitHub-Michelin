...
tags: [media-processing, imagemagick, tool]
language: C
license: NOASSERTION
maturity: active, ~16,892 stars (as of 2026-07)
last_verified: 2026-07-07
type: tool
upstream:
  pushed_at: 2026-07-06T06:03:09Z
  default_branch: main
  default_branch_sha: 414297f01da23ce5db7ced6da7f05d973910afc7
  archived: false
health:
  schema: 1
  computed_at: 2026-09-22T17:47:47Z
  overall: B
  overall_score: 3.4
  scored_axes: 5
  applicable_axes: 6
  ...
  axes:
    ...
    adoption:
      ...
      raw:
        ...
        homebrew_installs_90d: 176201
        homebrew_tier: A
        release_downloads: 4717879
        release_assets: 1088
        release_tier: B
...
You're choosing open-source infrastructure for a task that falls into `media-processing` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for ImageMagick when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because ImageMagick was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on ImageMagick.
...
|---|---|---|---|
| [ffmpeg-python](../video-audio/ffmpeg-python.md) | ✅ | When you need the established in-index option for this category, compare it against ImageMagick before switching. | ImageMagick is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose ImageMagick only after verifying the repo-specific caveats below. |
| [FFmpeg](../video-audio/ffmpeg.md) | ✅ | When you need the established in-index option for this category, compare it against ImageMagick before switching. | ImageMagick is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose ImageMagick only after verifying the repo-specific caveats below. |
| [ffsubsync](../video-audio/ffsubsync.md) | ✅ | When you need the established in-index option for this category, compare it against ImageMagick before switching. | ImageMagick is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose ImageMagick only after verifying the repo-specific caveats below. |
| [GStreamer](../video-audio/gstreamer.md) | ✅ | When you need the established in-index option for this category, compare it against ImageMagick before switching. | ImageMagick is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose ImageMagick only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-07-06T06:03:09Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-07-06T06:03:09Z` as of 2026-07-07.
- **Adoption snapshot:** ~16,892 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
