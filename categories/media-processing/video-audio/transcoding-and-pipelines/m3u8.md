...
type: library
upstream:
  pushed_at: 2025-01-31T21:13:34Z
  default_branch: master
  default_branch_sha: bfbfd48fccf446471dde0731f68f23d6c8299301
  archived: false
health:
  schema: 1
  computed_at: 2026-09-19T08:53:07Z
  overall: C
  overall_score: 1.67
  scored_axes: 3
  capped: false
...
|---|---|---|---|
| Hand-rolled regex / manual parsing | 未收录 | Choose hand-rolled parsing only when you need no dependency and accept reimplementing HLS grammar. | No dependency, but you reimplement the tag grammar, variant/media structure, and serialization — and HLS has many edge cases this library already handles. |
| hls.js / other-language parsers | 未收录 | Choose hls.js or other-language parsers when you need mature HLS parsing outside Python. | Mature HLS parsers in other runtimes (JS, Go); same job, wrong language if you're in Python. |
| [FFmpeg](ffmpeg.md) | ✅ | Choose FFmpeg when HLS read/write is part of media (de)muxing, not manifest object manipulation. | Reads/writes HLS as part of (de)muxing media, but it's an opaque media engine — not a manipulable manifest *object model* you can introspect and rewrite tag-by-tag. |
| streamlink / yt-dlp (as consumers) | 未收录 | Choose streamlink or yt-dlp when you need full stream-download tools that parse HLS internally. | Full stream-download tools that parse HLS internally; use them to *grab* a stream, not as a library to *manipulate* manifests in your own code. [未验证] |
