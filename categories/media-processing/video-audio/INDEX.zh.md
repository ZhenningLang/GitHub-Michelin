...
| **HandBrake** | 开源视频转码器，用于将几乎任意格式的视频转换为现代广泛支持的编解码器——基于 FFmpeg、x264 和 x265 构建，带有预设驱动的 GUI 和配套的 `HandBrakeCLI` 命令行工具。 | A（5/6） | [→](handbrake.zh.md) |
...
| 选项 | 是否收录 | 健康度 | 一句话取舍 |
| --- | --- | --- | --- |
| [claude-video](claude-video.zh.md) | ✅ | C（6/6） | 面向 agent 的视频理解 helper；视频生产 / 剪辑看 FFmpeg / MoviePy，只转录看 Whisper。 |
| [Auto-Editor](auto-editor.zh.md) | ✅ | A（6/6） | 当反复出现的任务是「删掉冷场、给我一条时间线」时选它；用 FFmpeg 要自己写判断逻辑，HandBrake 根本检测不了静音，MoviePy 则是要你编程的库。 |
