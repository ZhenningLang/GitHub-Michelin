...
| --- | --- | --- | --- |
| [OpenAI Whisper](whisper.md) | ✅ | B (5/6) | Pick it when you need the transcript itself, with model-size/quality tradeoffs you control; pick a bundled CLI path (for example [Auto-Editor](../editing-and-cutting/auto-editor.md)'s `whisper` subcommand) when the transcript is only an input to the next step. |
...
| [claude-video](claude-video.md) | ✅ | C (4/6) | Pick it when an agent needs to *watch* a video (frames plus captions as evidence); pick [OpenAI Whisper](whisper.md) when the transcript alone is the deliverable, because claude-video is a harness workflow, not an ASR engine. |
| [Auto-Editor](../editing-and-cutting/auto-editor.md) | ✅ | B (6/6) | Pick it when the transcript's purpose is to drive cutting (`--edit word:` / `--edit subtitle`); pick [OpenAI Whisper](whisper.md) when you need transcription quality knobs and nothing else. |
