...
health:
  schema: 1
  computed_at: 2026-09-19T09:34:52Z
  overall: C
  overall_score: 2.0
  scored_axes: 5
  capped: false
...
- **You only need scripted, repeatable renders with no interactive editing.** A GUI editor is the wrong shape — use [MoviePy](../video-audio/moviepy.md) for a Python batch pipeline, or [Remotion](../../video-production/remotion.md) when the video must be deterministic components rendered in CI.
- **You are building your own editor or a headless editing service.** Do not fork a young beta for its timeline model; use [MLT](../video-audio/mlt.md), the LGPL engine beneath Shotcut and Kdenlive, because it gives you timeline semantics without inheriting an AGPL application.
...
- **The edit is one commodity step inside a larger pipeline.** Concat documents no headless render mode; drive [FFmpeg](../video-audio/ffmpeg.md) directly instead, because a desktop application cannot be scheduled as a batch job.
