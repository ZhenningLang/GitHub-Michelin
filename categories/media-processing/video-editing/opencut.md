...
- **You only need programmatic, deterministic video from code.** A timeline editor is the wrong layer — use [Remotion](../../video-production/remotion.md) for React compositions rendered in CI, or [MLT](../video-audio/mlt.md) if you are building the editor yourself.
...
| [MLT](../video-audio/mlt.md) | ✅ | When you are building an editor rather than using one, pick MLT; pick OpenCut only to follow its architecture, because MLT is the mature LGPL engine that already powers shipped editors while OpenCut's rewrite has no announced date. | MLT: proven engine, no GUI, LGPL. OpenCut: full application and browser reach, unshipped. |
