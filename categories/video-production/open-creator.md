...
- **You cannot take a GPL-3.0 media core next to an Apache-2.0 shell.** Root `LICENSE` is Apache-2.0; `runtime/krillinai/LICENSE` is GNU GPL v3. If that mix is a hard no for redistribution, drive [FFmpeg](../media-processing/video-audio/transcoding-and-pipelines/ffmpeg.md) + [yt-dlp](../media-download/yt-dlp.md) yourself, or stay on MIT [MoneyPrinterTurbo](moneyprinter-turbo.md).
