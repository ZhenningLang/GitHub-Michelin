...
- **风险标记：** **PolyForm Noncommercial 1.0——不是 OSI 定义的开源。** 你产出的视频归你所有，但工具包的商用需作者授权；两个商业授权咨询 issue 已关闭，未公布商用授权渠道。[未验证] 打包字体单独以 SIL OFL-1.1 授权。英文配音默认依赖较脆（edge-tts 因跟踪微软接口而 pin 在 7.2.8；kokoro 在 ARM 上难装）。
...
- [未验证] 两个已关闭的商业授权 issue（#5、#6）关闭时无公开结论；能否取得商用授权未知。
