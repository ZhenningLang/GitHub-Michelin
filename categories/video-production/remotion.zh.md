...
用 React 程序化做视频的框架——composition 就是 React 组件，帧经 headless Chrome 渲染，Remotion Lambda 把渲染分布到 AWS——采用 source-available 许可证，个人与 3 人以下公司免费。
...
- 可选：AWS 账号（Lambda 渲染）；3 人以上组织需 `@remotion/licensing` 公司许可证 key。
...
- **维护（2026-09）：** 顶级——创建于 2020-06，每日有 push，v4.0.526 发布于 2026-09-17，多年来约 2 天一个 release；npm 首发 2020-12。
- **治理 / bus factor：** 组织（remotion-dev）即背后公司持有；约 30 名贡献者，创始人（JonnyBurger）占约 36.7k 次 commit 中的 31k（≈85%）——创始人主导，但创始人**就是**商业主体，维护激励是结构性的。
- **背书与 Lindy：** 6 年以上仍在加速——按本索引的先验是强 Lindy 画像；靠公司许可证而非 VC 级热度输血 [推断]。
- **采用与生态：** 59.7k stars / 4.6k forks；模板目录庞大，player 组件被生产级 web 应用使用，Lambda 渲染成熟；「视频即 React 代码」的默认答案。
- **风险信号：** source-available 许可证（非 OSI）带资格阈值，且 5.0 条款变更已公告；179 个 open issue 更多反映高使用量而非失修 [推断]；无「突然收紧许可证」的历史——付费档从项目早期就存在。

## 存疑（未验证）

- [未验证] 「Remotion 自带 FFmpeg 二进制」反映项目长期的捆绑行为；v4 的确切机制（安装时下载还是内置）未在此核查。
- [未验证] 公司许可证定价与 5.0 许可证的具体变更——做许可决策前请到 remotion.dev 核对现行条款；LICENSE.md 只写了资格分层。
- [未验证] star/fork/issue/贡献者数字（59.7k / 4.6k / 179 / 约 30）为 GitHub 时点数据（2026-09-18），易变。
