...
type: framework
upstream:
  pushed_at: 2026-09-18T10:30:32Z
  default_branch: main
  default_branch_sha: 70864064b18cc9c7cafc86a9b0fa7e856c8b22dc
  archived: false
health:
  schema: 1
  computed_at: 2026-09-18T10:51:09Z
  overall: A
  overall_score: 3.5
  scored_axes: 4
  capped: false
...
A React framework for making videos programmatically — compositions are React components, frames render through headless Chrome, and Remotion Lambda distributes renders across AWS — under a source-available license that is free for individuals and companies up to 3 employees.
...
- Optional: AWS account (Lambda renders), `@remotion/licensing` company license key for organizations >3 employees.
...
- **Maintenance (2026-09):** exceptional — created 2020-06, pushed daily, v4.0.526 released 2026-09-17, releases every ~2 days for years; first npm publish 2020-12.
- **Governance / bus factor:** org-owned (remotion-dev) by the company behind it; ~30 contributors with the founder (JonnyBurger) holding ~31k of ~36.7k commits (≈85%) — a founder-dominated repo, but the founder *is* the commercial entity, so incentives to maintain are structural.
...
- **Adoption & ecosystem:** 59.7k stars / 4.6k forks; large template catalog, player component used in production web apps, established Lambda rendering; the default answer for "video as React code".
- **Risk flags:** source-available license (non-OSI) with an eligibility threshold and an announced 5.0 terms change; 179 open issues reflects high usage more than neglect [推断]; no relicense-to-restrictive shock so far — the paid tier has existed since the project's early years.
...
- [未验证] Company-license pricing and the precise 5.0 license changes — verify current terms at remotion.dev before a licensing decision; LICENSE.md only states the eligibility tiers.
- [未验证] Star/fork/issue/contributor counts (59.7k / 4.6k / 179 / ~30) are point-in-time GitHub figures (2026-09-18); volatile.
