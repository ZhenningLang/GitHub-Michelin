...
- **Adoption: Grade A.** The automated axis found 8,155 monthly npm downloads and 0 dependent repositories for `@plantuml/core`. This measures the newer browser package, not the long-established JAR, server, IDE plugins, or documentation integrations; Homebrew and release-download signals fill that gap, so the grade reflects rather than understates the project's broader distribution surface.
