...
| nanoGPT (karpathy) | 未收录 | When English GPT-2 pretraining plus SFT is all you need and you want the smallest canonical reference, pick nanoGPT; pick MiniMind when you need the Chinese data path, MoE, tool-call SFT and the RL/RLAIF stages nanoGPT never covers, because nanoGPT stops where MiniMind's interesting half begins. | nanoGPT is smaller and cleaner as a pretraining reference; MiniMind is the only one of the two that reaches RLHF, MoE and agentic RL — at the cost of a much larger surface. |
