...
| **tusd** | 当你需要一个基于协议的稳健断点续传上传服务器，能把文件流式写到本地磁盘或云存储时用它——但它是独立服务，不是 NGINX 模块，对小文件/可靠网络可能过度设计。 | B（6/6） | [→](tusd.zh.md) |
...
| [tusd](tusd.zh.md) | ✅ | B（6/6） | 当你需要一个基于协议的稳健断点续传上传服务器，能把文件流式写到本地磁盘或云存储时用它——但它是独立服务，不是 NGINX 模块，对小文件/可靠网络可能过度设计。 |
