...
- **维护活跃度——安静，而且比 `pushed_at` 显示的更安静（截至 2026-09-20）。** GitHub 的 `pushed_at` 为 2026-09-18T22:12:00Z，但该字段也会因非默认分支与机器人活动而变动：实测默认分支上最后一次提交距今 **77 天**，最近 13 周里只有 **1 周**有活动，这也是雷达把维护活跃度评为 `B` 的原因。发布更稀疏——3.1.1（2025-08-29）、3.1.0（2024-10-18）、3.0.1（2024-02-12）——而未关闭 issue 只有 **19** 个，评分器干脆找不到合格的首次响应窗口（`?`）。请把它读作进入低干预维护模式的稳定项目，而不是被放弃的项目。
