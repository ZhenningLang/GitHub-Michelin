...
用于构建 Web 应用的前端框架与元框架——包括内容站与文档站框架（Astro、Docusaurus、Nextra），它们是「产出物是一个站点」的 React 元框架。不含组件库与设计系统（见 `web-ui/component-libraries`），不含浏览器自动化（见 `web-automation`），也不含产出 PDF 而非可运行站点的排版与文档编译器（见 `typesetting`）。
