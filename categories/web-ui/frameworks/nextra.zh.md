...
- **采用与生态——规模可观，但集中在 Next.js 世界。** 评分器把采用广度解析到 npm 上的包：**2,302 个依赖仓库、月下载 817,763**（`B`）——两项都比 Docusaurus 低一个数量级——另有约 13.9k star、约 1.4k fork。这里的采用是真实的，但更窄，且依赖 Next.js 生态维持现状。
