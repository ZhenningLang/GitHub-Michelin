...
- **采用与生态——规模很大，且有分发证据佐证。** 评分器把采用广度解析到 npm 上的 `@astrojs/internal-helpers`：**18,705 个依赖仓库、月下载 18,612,583**；仓库本身约 62.7k star、约 3.8k fork，另有主题与起始模板展示库、覆盖主流托管的官方适配器，以及从 Docusaurus、Hugo、Jekyll、Next.js 迁移的成文路径——后者本身就说明有多少流量在往这个方向走。
