...
health:
  schema: 1
  computed_at: 2026-09-22T11:38:15Z
  overall: A
  overall_score: 3.5
  scored_axes: 4
  capped: false
...
- **The complaint is token spend, not readability.** Use [caveman](caveman.md) when the goal is visibly fewer output tokens and you accept its benchmark caveats. i-have-adhd's list rule is explicitly presentation-only (it may not limit analysis, search, tool results, or candidate generation), so it is not a cost contract — and its always-on path *adds* input tokens by re-injecting the ruleset.
- **You are de-AI-ing a document you will publish.** Use [stop-slop](../de-ai-writing/stop-slop.md) or [humanizer](../de-ai-writing/humanizer.md). Those operate on prose artifacts; this one operates on chat replies, and neither substitutes for the other.
...
|---|---|---|---|
| [caveman](caveman.md) | ✅ | When the failure is reply *shape* for a reader with a short working memory, pick i-have-adhd; when the failure is visible token spend in general, pick caveman. | i-have-adhd adds cross-turn behaviour no brevity overlay has (restate progress, make wins visible, keep a real hedge) at the cost of a longer ruleset that adds input tokens when always-on; caveman is a shorter, broader style overlay. |
...
| [humanizer](../de-ai-writing/humanizer.md) | ✅ | Same axis as stop-slop, English-first: choose either for written output, not for chat replies. | Humanizer targets AI tells in published prose with plugin/install docs; i-have-adhd targets live conversation, where the AI tell is preamble and a closing pleasantry. |
