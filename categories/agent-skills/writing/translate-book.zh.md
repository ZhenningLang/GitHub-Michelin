...
| [Baoyu Skills](baoyu-skills.zh.md) | ✅ | 翻译只是内容流水线里一环（排版、发布、配图）时选 Baoyu Skills；任务明确就是一整本书时选 translate-book。 | Baoyu 的 `baoyu-translate` 是带术语表的三模式文本翻译 skill，不是带分块、manifest 校验和电子书输出的整书流水线——为一本书装上 20+ 个 skill 的合集，形状不对。 |
