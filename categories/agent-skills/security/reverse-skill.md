...
| PentestGPT (未收录) | ❌ | Pick PentestGPT when you want an academic auto-pentest agent design to study (paper lineage, ~15.5k stars, still occasionally updated as of 2026-07); pick reverse-skill when you want CI-checked routing across 45 domains with an engagement workflow rather than a research prototype. | PentestGPT is a research artifact with a paper but a thin harness around it; reverse-skill is workflow-complete but lacks academic documentation. |
| HexStrike AI (未收录) | ❌ | Pick HexStrike AI when your need is wiring many security tools to an agent through MCP execution (~11.9k stars, MIT, active as of 2026-08); pick reverse-skill when methodology selection, scope gating, and evidence reporting matter more than raw tool count. | HexStrike optimizes tool-execution coverage; reverse-skill optimizes workflow governance — overlapping means, different centers of gravity. |
...
- **Maintenance (2026-09):** active — last push 2026-09-03, single release v1.0.1, CHANGELOG kept. The routing benchmark is verified in-repo: `skills/config/routing.json` holds 44 priority rules and `skills/tests/routing-benchmark.json` holds exactly 175 cases, run in CI on Windows + Ubuntu; however the macOS test path is currently broken (#135, open).
...
- **Risk flags (the load-bearing part of this page):** (1) dual-use payload corpus — the WAF-bypass payload file `skills/pentest-tools/src-hunter/references/payloader/waf-bypass.md` is confirmed present in the repo tree (58 files under `payloader/`), and a user reported Defender flagging it as `Backdoor:PHP/ImagePHPBackdoor.A` (#125); the release zip has triggered virus warnings (#82); (2) open agent-safety concern — `README_AI.md` induces auto-execution/self-injection on first read (#134); (3) client-policy friction — documented refusals from AI clients (#86, #127); (4) install-path incident — bootstrap corrupted a Codex config with non-ASCII paths (#98, fixed). MIT license, no relicense risk. None of these are disqualifying for a lab tool; all of them are disqualifying for unmanaged corporate rollout.
...
- Verified in-repo (2026-09-17): 44 routing rules (`skills/config/routing.json`), 175 benchmark cases (`skills/tests/routing-benchmark.json`), and the flagged payload file `skills/pentest-tools/src-hunter/references/payloader/waf-bypass.md` all exist — kept here as audit trail.
...
- [未验证] Issue #125's specific Defender signature (`Backdoor:PHP/ImagePHPBackdoor.A`) is one user's report; the presence of the payload corpus itself is verified, the AV verdict is not independently reproduced.
