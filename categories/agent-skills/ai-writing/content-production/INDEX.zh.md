# content-production

> [ai-writing](../INDEX.zh.md) 的叶子。中文内容生产工具箱与流水线——把选题变成文章、创作者内容，并输出多种格式。
> ← 上层 [ai-writing](../INDEX.zh.md) · 根 [路由](../../../../INDEX.zh.md) · English: [INDEX.md](INDEX.md)

## 本叶子的合集

| 合集 | 何时用 | 健康度 | 页面 |
| --- | --- | --- | --- |
| **Baoyu Skills** | 宝玉出品的 20+ 个 coding agent 技能合集（翻译、markdown/HTML 排版、字幕与网页抓取、图片/图表/幻灯片生成），可装入 Claude Code、Codex 等支持 skill 的 harness。 | B（4/6） | [→](baoyu-skills.zh.md) |
| **huashu-skills** | 中文创作者工具箱，含 21 个文章、选题、调研、视频大纲、配图、PDF 导出和创作流程 skill。 | D（4/6） | [→](huashu-skills.zh.md) |
| **writing-agent** | 中文长文生产线，包含分阶段策划、证据账本、审稿、去 AI 味、事实核查闸门和最终 clean text。 | B（4/6） | [→](writing-agent.zh.md) |

## 对比矩阵

| 选项 | 是否收录 | 健康度 | 一句话取舍 |
| --- | --- | --- | --- |
| [Baoyu Skills](baoyu-skills.zh.md) | ✅ | B（4/6） | 20+ 个通用内容／排版工具合集；够宽，但不是小说或营销专用流水线。 |
| [huashu-skills](huashu-skills.zh.md) | ✅ | D（4/6） | 宽中文创作者工具箱；许可证不清和 per-skill 安装路径让风险更高。 |
| [writing-agent](writing-agent.zh.md) | ✅ | B（4/6） | 严格的中文文章生产线，带证据账本；比模块化创作者工具更重。 |

## 什么该放这里

面向**中文内容生产**的技能——文章／创作者写作、排版，以及多渠道内容工具。虚构写作放 [fiction](../fiction/INDEX.zh.md)，整书翻译放 [translation](../translation/INDEX.zh.md)，增长营销放 [marketing-seo](../marketing-seo/INDEX.zh.md)。
