...
- **卡片上有两项读法需要解码：两个 `?`。** 响应速度未评分，因为评分器找不到合格的首次响应窗口；risk_license 未评分，因为 GitHub 的 license API 报 `NOASSERTION`，许可文本无法被自动解析——尽管 `LICENSE` 文件明明白白就是 MIT。采用广度评为 `A`：**依赖图**信号显示依赖方数量达 6,586，同一次测量的**上个月下载量为 56,069,948**，两项都指向广泛采用。`[推断]`
