...
- **治理与维护者分散度——核心很小但确实共享，卡片也是这个结论。** 仓库属于 `typst` 组织（建于 2020-06-29，35 个公开仓库）。按最近 12 个月测量有 **43 位活跃贡献者**，**top1 占比 0.414**、**top3 占比 0.681**；历史总贡献前三为 `laurmaedje`（2,551）、`reknih`（215）、`saecki`（193）。历史上创始人主导，近期活跃度上则确实共享——比单维护者项目更结实，也是这一轴目前评级为 `?`（数据不足未评分）而非 `D` 的原因。
