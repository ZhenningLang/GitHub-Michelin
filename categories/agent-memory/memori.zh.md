...
health:
  schema: 1
  computed_at: 2026-09-19T08:47:47Z
  overall: B
  overall_score: 3.0
  scored_axes: 5
  capped: false
...
| Letta (MemGPT) | 未收录 | 需要把记忆管理 OS 放进有状态 agent runtime 时，选 Letta。 | 带记忆管理 OS 的 agent runtime（分层 context、自编辑记忆）；是更重的有状态 agent 服务，不是即插即用的客户端包裹器。 |
| Zep | 未收录 | 时序知识图谱记忆是主要架构下注时，选 Zep。 | 基于时序知识图谱的记忆服务，自托管/OSS 核心很强；结构化记忆的野心相当，但模型是图优先的。 |
| LangMem (LangChain) | 未收录 | 记忆工具需要绑定在 LangGraph/LangChain 栈内时，选 LangMem。 | 绑在 LangGraph/LangChain 栈上的记忆工具；独立性不如 Memori 的 framework-无关定位。 |
...
- **响应速度**：Grade A——中位首次响应时间 14.8 小时，基于 14 个 qualifying issues/PRs。
