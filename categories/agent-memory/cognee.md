...
language: Python
license: Apache-2.0
maturity: active, ~27,221 stars (as of 2026-07)
last_verified: 2026-07-06
type: service
upstream:
  pushed_at: 2026-07-06T01:16:39Z
  default_branch: main
  default_branch_sha: f7e2267cf02f5df15c4b60bf196b30ac2c06b32d
  archived: false
health:
  schema: 1
...
You're choosing open-source infrastructure for a task that falls into `agent-memory` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for Cognee when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because Cognee was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on Cognee.
...
- **Upstream state:** default branch `main`, last pushed `2026-07-06T01:16:39Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-07-06T01:16:39Z` as of 2026-07-06.
- **Adoption snapshot:** ~27,221 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
