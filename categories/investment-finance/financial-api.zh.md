...
- **你要的是没有供应商关系的取数。** 没有 Key 就没有数据——客户端里不附带任何数据集。预算必须为零时，选社区爬取源（AKShare、Tushare 免费积分档），并接受接口随时失效。
...
| AKShare | 未收录 | 预算严格为零、且能接受接口随时坏掉时选 AKShare；字段名稳定、参数有文档、来源官方这些值得换一把 Key 和供应商依赖时，选本页项目。 | AKShare 的广度（宏观、新闻、另类数据、港美股）超出本服务的公开范围，但它是社区爬取、没有契约、不承诺空值语义、也没有支持。 |
| Tushare | 未收录 | 结构上最接近的同类——托管 A 股数据服务加一个 token。看重积分制访问和长期 Python 社区时选 Tushare；要供应商官方数据加 MCP、CLI、Agent Skill 入口、而不是只有 Python 客户端时，选本页项目。 | Tushare 用更大的社区和更广的社区贡献面，换来纯 Python 的交互方式和积分经济学；本页不宣称与之逐接口对齐。 [未验证] |
...
- [未验证] Tushare 与 AKShare 的画像（配额模型、失效频率、广度）来自对生态的一般认知，本轮未重新核验。
