...
- **You must isolate untrusted, multi-tenant agent code.** Use a sandbox-per-session system such as [OpenSandbox](../agent-tooling/opensandbox.md), or HarnessRouter **Cloud** (serverless, isolated sandboxes); Community Edition runs every session as a separate OS user inside *one* shared container, which is a permission boundary, not a container boundary.
