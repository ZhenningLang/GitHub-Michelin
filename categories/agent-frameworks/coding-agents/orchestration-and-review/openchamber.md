...
- **Governance / bus factor — one person owns the critical path.** 225 contributors (including anonymous) across ~3,698 recorded contributions, but the top contributor holds ~64% of commits and the radar's 12-month window puts top-1 at 0.671 and top-3 at 0.736; the org (`openchamber`, created 2026-03) has 7 public repos and the copyright is held by a single person. An active community files fixes — the changelog credits outside contributors every release — yet the roadmap, merge queue, and release notes are his alone. [推断]
