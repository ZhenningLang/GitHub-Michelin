...
| 项目 | 何时用 | 健康度 | 页面 |
| --- | --- | --- | --- |
| **CC Switch** | 一款跨平台桌面 All-in-One 管理器，用于管理 Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw 和 Hermes Agent——基于 Rust 与 Tauri 2 构建。 | B（5/6） | [→](cc-switch.zh.md) |
...
| **OpenHands** | 🙌 OpenHands: AI-Driven Development | A（6/6） | [→](openhands.zh.md) |
| **RTK** | 高性能 CLI 代理，在命令输出到达 LLM 上下文前先过滤和压缩，对常见开发命令可减少 60–90% 的 token 消耗，开销低于 10 毫秒。 | A（6/6） | [→](rtk.zh.md) |
