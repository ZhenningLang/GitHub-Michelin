...
**核心思想是什么？**
别把材料塞进模型脑子，让它写程序去翻。材料当 Python 变量，子任务用 `rlm.spawn`。

**听起来很诡异。是不是没法充分利用 LLM 的智能？**
是。材料不进窗口，父模型变成调度员。怎么切块，反而需要你刚拿走的那种全局智能。他们自己的短数学实验也掉分。[推断]

**那他们在赌什么？**
长任务里窗口先腐烂，所以宁可付第二种成本。短任务、需要 A 和 B 同窗的活，这个赌是亏的。希望强化学习以后把模型训会这套脚手架。现在还没。[推断]
