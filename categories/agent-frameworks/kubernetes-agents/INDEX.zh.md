...
| 项目 | 何时用 | 健康度 | 页面 |
| --- | --- | --- | --- |
| **AX** | 当一大批空闲、有状态的 agent *任务*必须在 Kubernetes 上用 YAML 声明（工作区、出站、模型）、底下还能挂起／恢复时用它——不是把 agent 本身做成 CRD 的那条路。 | B（5/6） | [→](ax.zh.md) |
...
| 选项 | 是否收录 | 健康度 | 一句话取舍 |
| --- | --- | --- | --- |
| [AX](ax.zh.md) | ✅ | B（5/6） | 叠在密度运行时上、像 kubectl 的 Task YAML——对象在 Redis 不在 etcd，不带 agent 引擎，Substrate 是硬依赖。 |
