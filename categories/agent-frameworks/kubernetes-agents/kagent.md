...
- **You need many stateful agents multiplexed onto few machines.** kagent schedules agents as workloads; it does not snapshot-and-resume them to reclaim idle capacity. For density with suspend/resume of in-memory state, use [Agent Substrate](../../sandboxing/substrate.md).
- **You are not on Kubernetes, or you cannot install CRDs and an operator.** kagent's entire value is being cluster-native. Off Kubernetes, use a code-first framework such as [LangGraph](../agent-runtimes/langgraph.md) or [AgentScope](../agent-runtimes/agentscope.md).
...
| Kubernetes operator frameworks / agent CRD stacks in other projects | 未收录 | Choose a general operator framework when you are writing your own controller for agents; choose kagent when you want the agent CRDs, engine, tool servers and UI already built. | General frameworks (Kubebuilder, Operator SDK) are toolkits with no agent semantics; kagent ships the semantics but constrains your design to its resource model. Kept as out-of-scope here because the toolkit-vs-product comparison is a different selection question. |
