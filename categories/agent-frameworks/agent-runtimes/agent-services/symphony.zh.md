...
| GitHub Actions + agent CLI（自建） | 未收录 | 想自己搭 issue 触发的 agent 运行时，选 GitHub Actions 加 agent CLI。 | 自己搭：CI 在 issue 上触发 agent。控制力更强、基础设施更持久（CI runner），但 dispatch/对账/隔离得你自己造——这些正是 Symphony 给你的。 |
