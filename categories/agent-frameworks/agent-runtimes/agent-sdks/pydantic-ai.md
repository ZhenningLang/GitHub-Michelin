...
language: Python
license: MIT
maturity: active, ~18,243 stars (as of 2026-07)
last_verified: 2026-07-06
type: framework
upstream:
  pushed_at: 2026-07-06T14:40:38Z
  default_branch: main
  default_branch_sha: c0ce014143520d179ee81454f1bda736762d322d
  archived: false
health:
  schema: 1
...
You're choosing open-source infrastructure for a task that falls into `agent-runtimes` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for Pydantic AI when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because Pydantic AI was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on Pydantic AI.
...
|---|---|---|---|
| [AgentScope](agentscope.md) | ✅ | When you need the established in-index option for this category, compare it against Pydantic AI before switching. | Pydantic AI is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Pydantic AI only after verifying the repo-specific caveats below. |
| [AutoGen](autogen.md) | ✅ | When you need the established in-index option for this category, compare it against Pydantic AI before switching. | Pydantic AI is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Pydantic AI only after verifying the repo-specific caveats below. |
| [CrewAI](crewai.md) | ✅ | When you need the established in-index option for this category, compare it against Pydantic AI before switching. | Pydantic AI is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Pydantic AI only after verifying the repo-specific caveats below. |
| [Hermes Agent](../personal-assistants/hermes-agent.md) | ✅ | When you need the established in-index option for this category, compare it against Pydantic AI before switching. | Pydantic AI is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose Pydantic AI only after verifying the repo-specific caveats below. |
...
- **Adoption snapshot:** ~18,243 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
