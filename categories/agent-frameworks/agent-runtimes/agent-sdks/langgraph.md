...
language: Python
license: MIT
maturity: active, ~36,628 stars (as of 2026-07)
last_verified: 2026-07-06
type: framework
upstream:
  pushed_at: 2026-07-05T00:59:26Z
  default_branch: main
  default_branch_sha: be999ad38a8443a2a64d468e33c1228ca5aede4f
  archived: false
health:
  schema: 1
  computed_at: 2026-07-06T15:17:30Z
  overall: A
  overall_score: 3.67
  scored_axes: 6
  capped: false
...
You're choosing open-source infrastructure for a task that falls into `agent-runtimes` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for LangGraph when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because LangGraph was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on LangGraph.
...
|---|---|---|---|
| [AgentScope](agentscope.md) | ✅ | When you need the established in-index option for this category, compare it against LangGraph before switching. | LangGraph is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose LangGraph only after verifying the repo-specific caveats below. |
| [CrewAI](crewai.md) | ✅ | When you need the established in-index option for this category, compare it against LangGraph before switching. | LangGraph is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose LangGraph only after verifying the repo-specific caveats below. |
| [Hermes Agent](../personal-assistants/hermes-agent.md) | ✅ | When you need the established in-index option for this category, compare it against LangGraph before switching. | LangGraph is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose LangGraph only after verifying the repo-specific caveats below. |
| [OpenClaw](../personal-assistants/openclaw.md) | ✅ | When you need the established in-index option for this category, compare it against LangGraph before switching. | LangGraph is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose LangGraph only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-07-05T00:59:26Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-07-05T00:59:26Z` as of 2026-07-06.
- **Adoption snapshot:** ~36,628 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
