...
- 文档里最诚实的一句是「能写成函数的任务就去写函数，别用 AI agent」。一个框架在自家总览里告诉你何时别用它，比想象中稀缺。
- agent 优先还是图优先是真的模型差异，不是教程风格：这里默认单位是会自己循环的 `Agent`，图是可选的；LangGraph 里编译好的图就是程序。以后想在两者之间搬家，是重构不是翻译。[推断]
