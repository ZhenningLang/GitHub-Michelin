...
language: Python
license: CC-BY-4.0
maturity: active, ~59,526 stars (as of 2026-07)
...
You're choosing open-source infrastructure for a task that falls into `agent-runtimes` and you need a real repository to evaluate, not just a product name from a comparison table. You reach for AutoGen when its upstream description matches the job and when adopting an existing project is preferable to writing custom glue from scratch.

This first-pass page exists because AutoGen was repeatedly useful as a comparison candidate in the atlas backlog. Use it as an intake-backed starting point: verify the upstream README and license, then compare it against the linked neighboring pages before committing to the dependency.
...
- **The GitHub metadata flags a blocker for your environment.** If license, archival status, or maintenance cadence is load-bearing, choose a better-verified alternative in this category instead of relying on AutoGen.
...
|---|---|---|---|
| [AgentScope](agentscope.md) | ✅ | When you need the established in-index option for this category, compare it against AutoGen before switching. | AutoGen is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose AutoGen only after verifying the repo-specific caveats below. |
| [CrewAI](crewai.md) | ✅ | When you need the established in-index option for this category, compare it against AutoGen before switching. | AutoGen is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose AutoGen only after verifying the repo-specific caveats below. |
| [Hermes Agent](../personal-assistants/hermes-agent.md) | ✅ | When you need the established in-index option for this category, compare it against AutoGen before switching. | AutoGen is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose AutoGen only after verifying the repo-specific caveats below. |
| [LangGraph](langgraph.md) | ✅ | When you need the established in-index option for this category, compare it against AutoGen before switching. | AutoGen is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose AutoGen only after verifying the repo-specific caveats below. |
...
- **Upstream state:** default branch `main`, last pushed `2026-04-15T11:59:09Z`, archived `false`.
...
- **Maintenance snapshot:** GitHub reports `archived=false` and `pushed_at=2026-04-15T11:59:09Z` as of 2026-07-06.
- **Adoption snapshot:** ~59,526 GitHub stars as of 2026-07; stars are only a noisy adoption signal.
- **License snapshot:** `CC-BY-4.0` from GitHub API; inspect repository license files when the license matters.
