...
| [AX](../agent-frameworks/kubernetes-agents/ax.md) | ✅ | Choose AX when you want kubectl-shaped Task/Workspace YAML and will take Substrate as a dependency; choose Substrate when you are building that control plane yourself and need to speak WorkerPool/Actor directly. | AX is the developer-facing orchestrator that stores tasks in Redis and provisions Substrate actors; Substrate is the density runtime underneath. They stack — AX is not a substitute for snapshot/resume. |
