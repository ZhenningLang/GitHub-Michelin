...
- **You need your daily logged-in browser.** It launches Playwright-managed browser instances (persistent `--user-data-dir` profiles and `--cdp-endpoint` attach exist, but the default is an automation browser, not your everyday Chrome with its cookies, 2FA sessions and risk-control reputation). For agents that must operate your real logged-in session, [OpenCLI](opencli.md) or [Agent Browser](agent-browser.md)'s session persistence are closer fits.
