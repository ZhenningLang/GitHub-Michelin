...
| [Playwright MCP](playwright-mcp.zh.md) / [Playwright CLI](playwright-cli.zh.md) | ✅ | 要厂商官方、对一次性浏览器的确定性自动化时选 Playwright 两兄弟；当「浏览器就是我日常在用的那个」是硬需求时选 OpenCLI。 | Playwright 更干净且跨浏览器，但每个流程都从未认证的自动化 profile 起步；OpenCLI 只支持 Chromium 桌面但身份零成本。 |
