...
- **你想要轻量的 CDP 调试/测量工具。** 要做 Chrome 原生的性能 trace、网络/控制台检查、堆快照并由 agent 驱动，像 [Chrome DevTools MCP](../agent-browser-tools/chrome-devtools-mcp.zh.md) 这类 CDP 工具，比架起 WebDriver + Grid 轻得多。
...
- **响应速度**：Grade A——中位首次响应时间 21.8 小时，基于 40 个 qualifying issues/PRs。
...
- **采用与生态** —— 依赖图仍然读作 E（记录到 0 个依赖仓库），但这个口径会低估分散在多语言绑定、浏览器驱动、托管 Grid 和 CI 集成里的 WebDriver 生态。下载量给出了相反的结论，并且把这一轴拉到了 A：每月 128,246,077 次。人工阅读时应把 Selenium 视为深度扎根的生态；在这里包依赖图是用错了的量具。`[推断]`
