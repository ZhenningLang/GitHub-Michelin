...
| [Cua](cua.md) | ✅ | Choose Cua when the task leaves the browser — native desktop apps, OS dialogs, VM isolation; choose Agent Browser when everything happens on web pages and determinism matters more than reach. | Cua is vision-based computer-use in a VM; Agent Browser is browser-only and a11y-tree-based, which is cheaper and more reproducible but blind outside the page. |
