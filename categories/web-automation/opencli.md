...
| [Playwright MCP](playwright-mcp.md) / [Playwright CLI](playwright-cli.md) | ✅ | Pick the Playwright pair for vendor-official, deterministic automation of disposable browsers; pick OpenCLI when "the browser is the one I already use" is the requirement. | Playwright is cleaner and cross-browser but starts every flow from an unauthenticated automation profile; OpenCLI is Chromium/desktop-only but identity comes free. |
