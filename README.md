...
| **Plannotator** | Use it when a human must annotate or approve what the agent produced — a plan, a diff, an HTML artifact — and send that markup back as the agent's next instruction; but it's 9 months old, pre-1.0, single-maintainer, and its OSS team-sharing path is being superseded by a hosted product. | MIT OR Apache-2.0 | B (6/6) | [EN](categories/agent-tooling/plannotator.md) · [中](categories/agent-tooling/plannotator.zh.md) |
### sandboxing
