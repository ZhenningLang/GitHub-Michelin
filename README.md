...
| **Auto-Editor** | Use it when the first pass is mechanical — cut silence by loudness (or cut by the spoken words) in one command, then render a trimmed file or export an importable Premiere / Resolve / Final Cut timeline — but it's a UI-less CLI from one maintainer whose PyPI channel is retired. | Unlicense | B (6/6) | [EN](categories/media-processing/video-audio/auto-editor.md) · [中](categories/media-processing/video-audio/auto-editor.zh.md) |
